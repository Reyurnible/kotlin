...
public class LazyJavaClassMemberScope(
    ...
    private fun SimpleFunctionDescriptor.isVisibleAsFunctionInCurrentClass(): Boolean {
        if (getPropertyNamesCandidatesByAccessorName(name).any {
            propertyName ->
            // TODO: test overloads of setters
            getPropertiesFromSupertypes(propertyName).any {
                property ->
                doesClassOverridesProperty(property) {
                    if (this@isVisibleAsFunctionInCurrentClass.name == it)
                        listOf(this@isVisibleAsFunctionInCurrentClass)
                    else
                        searchMethodsByNameWithoutBuiltinMagic(it) + searchMethodsInSupertypesWithoutBuiltinMagic(it)
                } && (property.isVar || !JvmAbi.isSetterName(name.asString()))
            }
        }) return false
    ...
    override fun computeNonDeclaredFunctions(result: MutableCollection<SimpleFunctionDescriptor>, name: Name) {
        ...
        if (!name.sameAsRenamedInJvmBuiltin && !name.sameAsBuiltinMethodWithErasedValueParameters) {
            addFunctionFromSupertypes(result, name, functionsFromSupertypes.filter { it.isVisibleAsFunctionInCurrentClass() })
            return
        }
        ...
        addOverriddenBuiltinMethods(
                ...
                { searchMethodsInSupertypesWithoutBuiltinMagic(it) },
                {
                    ...
                    descriptor.copy(
                        ...
                        addOverriddenDescriptor(original)
                        specialBuiltinsFromSuperTypes.add(this)
                    }
                }
        )

        val visibleFunctionsFromSupertypes =
                functionsFromSupertypes.filter { it.isVisibleAsFunctionInCurrentClass() } + specialBuiltinsFromSuperTypes
    ...
    private fun addOverriddenBuiltinMethods(
        ...
        for (descriptor in candidatesForOverride) {
            ...
            ?.check { override -> override.first.isVisibleAsFunctionInCurrentClass() }
            ?.let { addFunction(it.first, it.second) }
        }
    }
    ...
    private fun getFunctionsFromSupertypes(name: Name): MutableSet<SimpleFunctionDescriptor> {
    ...
    private fun createPropertyDescriptorByMethods(
        ...
        val getter = PropertyGetterDescriptorImpl(
                getterMethod, propertyDescriptor
        ).apply {
            initialize(propertyDescriptor.type)
        }

        val setter = setterMethod?.let { setterMethod ->
            PropertySetterDescriptorImpl(setterMethod, propertyDescriptor).apply { initializeDefault() }
        }

        return propertyDescriptor.apply { initialize(getter, setter) }
    }

    private fun getPropertiesFromSupertypes(name: Name): MutableSet<PropertyDescriptor> {
        return getContainingDeclaration().typeConstructor.supertypes.flatMap {
    ...
    private fun hasOverriddenBuiltinFunctionWithErasedValueParameters(
        ...
        if (candidatesToOverride.isEmpty()) return false
        return candidatesToOverride.any overrides@{
            candidate ->
